...
/*	----------------------------												*/
/*  Subgroup analysis 3: Stimulants only										*/
/*  inclusion criteria:															*/ 
/*  1. at least one stimulants rx between 01JAN2001 and 31DEC2020				*/
...
quit; /*number of suicide events in ppl w/ adhd rx=851*/
...
*incident patients of outcome in study period = 4851;
...
group by &patientid;
quit;
*(2) create stimulant cohort from ADHD rx cohort;
proc sql;
create table b.cohort_stimulants as
select distinct &patientid from b.exp_stimulants
where &patientid in (select &patientid from b.cohort_final);
quit;
proc sql;
create table b.cohort_stimulants_2 as
select a.*,b.stist, b.stied, c.nstist, c.nstied  
from b.exp_data as a
left join b.exp_stimulants_2 as b
on a.&patientid=b.&patientid
left join b.exp_nonstimulants_2 as c
on a.&patientid=c.&patientid
where a.&patientid in (select &patientid from b.cohort_stimulants) ;
quit;
*(3) if stimulants switch to nonstimulants then first nonstimulants rxst=observation end for the patient;
data b.cohort_stimulants_3;
set b.cohort_stimulants_2;
where &nonstimulants;
run;
proc sql;
create table b.cohort_stimulants_4 as
select distinct (a.&patientid), a.&sex, a.&birth_date, a.&obsst, a.&obsed,
b.&trtst, b.&trted, 
(case when b.&trtst>b.stist then min(b.&trtst) else a.&obsed end) as obsed_2 format date9.
from b.cohort_final as a left join b.cohort_stimulants_3 as b
on a.&patientid=b.&patientid
group by a.&patientid;
quit;
proc sql;
create table b.cohort_stimulants_5 as
select distinct (&patientid), &sex, &birth_date, &obsst, obsed_2, &obsed
from b.cohort_stimulants_4 
where &patientid in (select &patientid from b.cohort_stimulants)
group by &patientid;
quit;
*(4) if nonstimulants switch to stimulants then last nonstimulants rxed=observation start for the patient;
proc sql;
create table b.cohort_stimulants_6 as
select distinct (a.&patientid), a.&sex, a.&birth_date, a.&obsst, a.&obsed,
b.&trtst, b.&trted, 
(case when b.&trted<b.stist then max(b.&trted) else a.&obsst end) as obsst_2 format date9.
from b.cohort_final as a left join b.cohort_stimulants_3 as b
on a.&patientid=b.&patientid
where a.&patientid in (select &patientid from b.cohort_stimulants)
group by a.&patientid;
quit;
proc sql;
create table b.cohort_stimulants_7 as
select distinct (&patientid), &sex, &birth_date, &obsst, obsst_2, &obsed
from b.cohort_stimulants_6 
where &patientid in (select &patientid from b.cohort_stimulants)
group by &patientid
;
quit;
proc sql;
create table b.cohort_stimulants_final as
select unique(a.&patientid), a.&sex, a.&birth_date, 
(case when b.&patientid in (select &patientid from b.cohort_stimulants_7) 
then b.&obsst else a.&obsst end) as obsst format date9.,
(case when c.&patientid in (select &patientid from b.cohort_stimulants_5) 
then c.&obsed else a.&obsed end) as obsed format date9.
from b.cohort_final as a left join b.cohort_stimulants_7 as b
on a.&patientid = b.&patientid
left join b.cohort_stimulants_5 as c
on a.&patientid = c.&patientid
where a.&patientid in (select &patientid from b.cohort_stimulants)
group by a.&patientid;
quit;
...
from b.exp_stimulants as a left join b.cohort2_4 as b on compress(a.&patientid)=compress(b.&patientid)
where a.&patientid in (select &patientid from b.cohort_stimulants_final)
...
from b.cohort_stimulants_final as a 
...
from b.cohort_stimulants_final as a left join b.&label._data as b on compress(a.&patientid)=compress(b.&patientid)
...
from b.cohort_stimulants_final as a left join b.&label._data as b on a.&patientid=b.&patientid
where a.&obsst<=b.&start_dt<=a.&obsed
...
var cosick1 cosick2 cosick3 cosick4 cosick5 cosick6 cosick7 cosick8 ;
...
max(cosick8) as p_cosick8
from b.dx_data
where &patientid in (select &patientid from b.cohort_stimulants_final) and &datea<=&admdt<=&dateb
group by &patientid;
quit;
...
from b.rx_data
where &patientid in (select &patientid from b.cohort_stimulants_final) and &datea<=&trtst<=&dateb
group by &patientid;
quit;
proc sql;
...
*Subgroup 3: stimulants analysis;
%macro stimulants(unit, reference);

data sccs_&unit.0;
set b.final_data;
by &patientid;
compare=compress(catx("-", outcome, &patientid, &unit., codrug1, codrug2, codrug3, codrug4, codrug5, nonstimulants, age, season));
...
select distinct outcome, &patientid, &unit., codrug1, codrug2, codrug3, codrug4, codrug5, nonstimulants,
...
season
from sccs_&unit.0
group by outcome, &patientid, &unit., codrug1, codrug2, codrug3, codrug4, codrug5, nonstimulants, age, season, order
...
model outcome=&unit. &patientid age codrug1-codrug5 nonstimulants season / dist=poisson offset=l_fu_year link=log;
ods output ParameterEstimates=result;
quit;

data d.irr_&unit._stim; 
set result;
IRR=exp(Estimate);
...
proc sql;
ods output SQL_results=d.incidence_&unit._stim;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "stimulants&unit" as table
from sccs_&unit.
group by &unit.);
quit;

%mend;
title "stimulants analysis";
%stimulants(exposure, 0);
%stimulants(riskperiod, 0);

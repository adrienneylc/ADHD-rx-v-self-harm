...
/*	----------------------------												*/
/*  Sensitivity analysis 2: Remove people with a record of death				*/
/*  inclusion criteria:															*/ 
...
data b.cohort2_5;
set b.cohort2_4;
if death_date < '31DEC2020'd and not missing (death_date)
then delete;
run;
...
from b.rx_data as a, b.cohort2_5 as b
...
*incident patients of outcome in study period = ;
...
/* calculatiing daily dose */
*1) calculating cumulative duration without duplicative period;
data duration;
set exp_data3;
run;
...
data duration2;
set duration;
rename &trtst=start &trted=stop;
label &trtst=start &trted=stop;
keep &patientid &trtst &trted;
run;

proc sql;
create table duration3 as
select &patientid, max(&datea, start) as start_dt format yymmdd10., min(stop, &dateb) as stop_dt format yymmdd10.
from duration2
order by &patientid, calculated start_dt;
quit;

data duration4;
set duration3;
if stop_dt < start_dt then delete;
run;
proc transpose data=duration4 out=start_dt (drop=_name_) prefix=start;
by &patientid;
var start_dt;
run;
proc transpose data=duration4 out=stop_dt (drop=_name_) prefix=stop;
by &patientid;
var stop_dt;
run;

data duration5;
merge start_dt stop_dt;
by &patientid;
run;

data b.exp_cduration;
retain TOTDUR;
set duration5; 
array START{*} START:;
array STOP{*} STOP:;
...
* calculating total dose of exposure;
proc sql;
create table exp_data4 as
select distinct b.&patientid, a.&trtst, a.&trted, b.&trtq, b.&trtdose, b.&trtday, b.&trtday*&trtdose*&trtq as tot_dose
from exp_data2 as a left join exp_data1 as b
on a.order=b.order
order by b.&patientid, a.&trtst;
quit;
...
proc sql;
create table b.exp_totdose as
select distinct &patientid, sum(tot_dose) as tot_dose
from exp_data4
group by 1;
quit;
proc means data=b.exp_totdose n median min max q1 q3;
var tot_dose;
run;
proc sql;
create table b.exp_dd as
select a.*, b.totdur, a.tot_dose/b.totdur as dd, totdur/365.25 as totdur_y
from b.exp_totdose as a left join b.exp_cduration as b
on a.&patientid=b.&patientid;
quit;

proc means data=b.exp_dd n median min max q1 q3;
var dd;
run;
...
4f. remove ppl with adhd rx 12 months before observation start (check with main analysis);

*Sensitivity analysis 2;
%macro sen2(unit, reference);
...
proc sql;
ods output SQL_results=d.incidence_&unit._sen2;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "sen2&unit" as table
from sccs_&unit.
group by &unit.);
quit;

%mend;
title "Sensitivity analysis 2: without people who died";
%sen2(exposure, 0);
%sen2(riskperiod, 0);

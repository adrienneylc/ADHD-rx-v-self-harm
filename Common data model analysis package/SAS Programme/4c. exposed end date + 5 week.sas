...
/*	----------------------------												*/
/*  Sensitivity analysis 4:	Extend exposed period by 5 weeks					*/
/*  inclusion criteria:															*/ 
...
proc sql;
ods output SQL_results=d.incidence_&unit._sen4;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "sen4&unit" as table
from sccs_&unit.
group by &unit.);
quit;

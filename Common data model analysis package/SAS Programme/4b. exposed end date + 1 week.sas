...
/*	----------------------------												*/
/*  Sensitivity analysis 3:	Extend exposed period by 1 week						*/
/*  inclusion criteria:															*/ 
...
*Sensitivity analysis 3. exposed end date + 1 week;
%macro sen3(unit, reference);
...
proc sql;
ods output SQL_results=d.incidence_&unit._sen3;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "sen3&unit" as table
from sccs_&unit.
group by &unit.);
quit;

%mend;
title "Sensitivity analysis 3: Extension of exposed period by 1 week ";
%sen3(exposure, 0);
%sen3(riskperiod, 0);

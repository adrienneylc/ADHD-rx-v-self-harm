...
libname a "D:\SAS\sccs_mph_NZ\raw"; /*original data*/
libname b "D:\SAS\sccs_mph_NZ\working library"; /*working library*/
libname c "D:\SAS\sccs_mph_NZ\common_data_model";  /* Please enter the path which save the coverted CDM */
libname d "D:\SAS\sccs_mph_NZ\results";  /* Folder which save the Results */
...
%let path=D:\SAS\sccs_mph_NZ; 			*Specify the pathname to your data folder;
%let datea0='01JAN2007'd;           * PLEASE UPDATE IF REQUIRED; *start date of study period, 01JAN2001'd;
%let datea='01JAN2007'd;           	* PLEASE UPDATE IF REQUIRED; *start date of observation period, 01JAN2001'd;
%let dateb='31DEC2019'd;          	* PLEASE UPDATE IF REQUIRED; *end date of observation period, 31DEC2020'd;
%let yeare=2006;                    * PLEASE UPDATE IF REQUIRED; *eligible period, for dbs without life-long enrolment;
%let yeara=2007;                    * PLEASE UPDATE IF REQUIRED; *start year of observation period, 2001;
%let yearb=2019;                    * PLEASE UPDATE IF REQUIRED; *end year of observation period, 2020; 
...
/*	----------------------------												*/
/*  Subgroup analysis 4: Non-Stimulants only									*/
/*  inclusion criteria:															*/ 
/*  1. at least one non-stimulants rx	between 01JAN2001 and 31DEC2020			*/
...
quit; /*number of suicide events in ppl w/ adhd rx=*/
title 'Number of patients with suicide attempt during study period ';
proc sql; 
select count(distinct &patientid) 
from b.outcome_all; 
quit; *prevalent patients of outcome = ;
...
*6) selecting incident patients of outcome during the study period from cohort5 -> cohort6;
...
*7) Remove exp and outcome same date;
data b.cohort5; 
set b.outcome_first; 
keep &patientid;
run; 
...
quit; *outcome and exposure date is equal = ;
proc sql;
...
select compress(b.&patientid) as id, b.&sex, b.&birth_date, c.&obsst, c.&obsed 
from b.cohort7 as a left join c.demo_final as b on compress(a.&patientid)=compress(b.&patientid)
left join b.cohort2_4 as c on compress(a.&patientid)=compress(c.&patientid)
order by 1;
quit;
proc sort data=b.cohort_8 out=b.cohort_final nodupkey; by _all_; run;
*8) apply stimulants-nonstimulants switch date as left or right censored day;
*(1) define first treatment start date and last treatment end date for stimulants & non-stimulants;
proc sql;
create table b.exp_stimulants as 
select* 
from b.exp_data
where &stimulants and &patientid in (select &patientid from b.cohort_final);
quit;
proc sql; 
create table b.exp_stimulants_2 as 
select distinct &patientid, 
min(&trtst) as stist format date9.,
max(&trted) as stied format date9.
from b.exp_stimulants
group by &patientid;
quit;

proc sql;
create table b.exp_nonstimulants as 
select* 
from b.exp_data
where &nonstimulants and &patientid in (select &patientid from b.cohort_final);
quit;
proc sql; 
create table b.exp_nonstimulants_2 as 
select distinct &patientid, 
min(&trtst) as nstist format date9.,
max(&trted) as nstied format date9.
from b.exp_nonstimulants
group by &patientid;
quit;
*(2) create nonstimulant cohort from ADHD rx cohort;
proc sql;
create table b.cohort_nonstimulants as
select distinct &patientid from b.exp_nonstimulants
where &patientid in (select &patientid from b.cohort_final);
quit;
proc sql;
create table b.cohort_nonstimulants_2 as
select a.*,c.stist, c.stied, b.nstist, b.nstied  
from b.exp_data as a
left join b.exp_nonstimulants_2 as b
on a.&patientid=b.&patientid
left join b.exp_stimulants_2 as c
on a.&patientid=c.&patientid
where a.&patientid in (select &patientid from b.cohort_nonstimulants) ;
quit;
*(3) if nonstimulants switch to stimulants then first stimulants rxst=observation end for the patient;
data b.cohort_nonstimulants_3;
set b.cohort_nonstimulants_2;
where &stimulants;
run;
proc sql;
create table b.cohort_nonstimulants_4 as
select distinct (a.&patientid), a.&sex, a.&birth_date, a.&obsst, a.&obsed,
b.&trtst, b.&trted, 
(case when b.&trtst>b.nstist then min(b.&trtst) else a.&obsed end) as obsed_2 format date9.
from b.cohort_final as a left join b.cohort_nonstimulants_3 as b
on a.&patientid=b.&patientid
group by a.&patientid;
quit;
proc sql;
create table b.cohort_nonstimulants_5 as
select distinct (&patientid), &sex, &birth_date, &obsst, obsed_2, &obsed
from b.cohort_nonstimulants_4 
where &patientid in (select &patientid from b.cohort_nonstimulants)
group by &patientid;
quit;
*(4) if stimulants switch to nonstimulants then last nonstimulants rxed=observation start for the patient;
proc sql;
create table b.cohort_nonstimulants_6 as
select distinct (a.&patientid), a.&sex, a.&birth_date, a.&obsst, a.&obsed,
b.&trtst, b.&trted, 
(case when b.&trted<b.nstist then max(b.&trted) else a.&obsst end) as obsst_2 format date9.
from b.cohort_final as a left join b.cohort_nonstimulants_3 as b
on a.&patientid=b.&patientid
where a.&patientid in (select &patientid from b.cohort_nonstimulants)
group by a.&patientid;
quit;
proc sql;
create table b.cohort_nonstimulants_7 as
select distinct (&patientid), &sex, &birth_date, &obsst, obsst_2, &obsed
from b.cohort_nonstimulants_6
where &patientid in (select &patientid from b.cohort_nonstimulants)
group by &patientid
;
quit;
proc sql;
create table b.cohort_nonstimulants_final as
select unique(a.&patientid), a.&sex, a.&birth_date, 
(case when b.&patientid in (select &patientid from b.cohort_nonstimulants_7) 
then b.&obsst else a.&obsst end) as obsst format date9.,
(case when c.&patientid in (select &patientid from b.cohort_nonstimulants_5) 
then c.&obsed else a.&obsed end) as obsed format date9.
from b.cohort_final as a left join b.cohort_nonstimulants_7 as b
on a.&patientid = b.&patientid
left join b.cohort_nonstimulants_5 as c
on a.&patientid = c.&patientid
where a.&patientid in (select &patientid from b.cohort_nonstimulants)
group by a.&patientid;
quit;
...
/******************************/
data b.basedata; 
set b.cohort_nonstimulants_final; 
...
from b.exp_nonstimulants as a left join b.cohort2_4 as b on compress(a.&patientid)=compress(b.&patientid)
where a.&patientid in (select &patientid from b.cohort_nonstimulants_final)
...
run;
/* Question 1: stt_flag definition ok?*/
data exp_data1; 
set exp_data1; 
by &patientid; 
...
else if riskperiod=. then riskperiod=0;
...
from b.cohort_nonstimulants_final as a 
left join b.outcome_data as b on compress(a.&patientid)=compress(b.&patientid)
...
from b.cohort_nonstimulants_final as a left join b.&label._data as b on compress(a.&patientid)=compress(b.&patientid)
...
from b.cohort_nonstimulants_final as a left join b.&label._data as b on a.&patientid=b.&patientid
where a.&obsst<=b.&start_dt<=a.&obsed
...
from b.dx_data
where &patientid in (select &patientid from b.cohort_nonstimulants_final) and &datea<=&admdt<=&dateb
group by &patientid;
quit;
...
max(mph) as p_mph, max(stimulants) as p_stimulants, max(nonstimulants) as p_nonstimulants
from b.rx_data
where &patientid in (select &patientid from b.cohort_nonstimulants_final) and &datea<=&trtst<=&dateb
group by &patientid;
quit;
proc sql;
...
b.date, b.exposure, b.sttdt_index, b.enddt_index, b.riskperiod
from b.basedata as a left join 
b.exp_final as b on compress(a.&patientid)=compress(b.&patientid) and a.date=b.date
order by a.&patientid, a.date;
quit;
proc sql;
...
left join b.outcome_final as b on compress(a.&patientid)=compress(b.&patientid) and a.date=b.date
order by a.&patientid, a.date;
quit;
proc sql;
...
left join b.codrug_final as b on compress(a.&patientid)=compress(b.&patientid) and a.date=b.date
order by a.&patientid, a.date;
quit;
proc sql;
...
left join b.cosick_final as b on compress(a.&patientid)=compress(b.&patientid) and a.date=b.date
order by a.&patientid, a.date;
quit;
proc sql;
...
left join b.age_final as b on compress(a.&patientid)=compress(b.&patientid) and a.date=b.date
order by a.&patientid, a.date;
quit;
proc sql;
...
left join b.outcome_first as b on compress(a.&patientid)=compress(b.&patientid) 
order by a.&patientid, a.date;
quit;
...
*Subgroup 4: non-stimulants analysis;
%macro nonstimulants(unit, reference);

data sccs_&unit.0;
set b.final_data;
by &patientid;
compare=compress(catx("-", outcome, &patientid, &unit., codrug1, codrug2, codrug3, codrug4, codrug5, stimulants, age, season));
...
select distinct outcome, &patientid, &unit., codrug1, codrug2, codrug3, codrug4, codrug5, stimulants,
...
season
from sccs_&unit.0
group by outcome, &patientid, &unit., codrug1, codrug2, codrug3, codrug4, codrug5, stimulants, age, season, order
...
model outcome=&unit. &patientid age codrug1-codrug5 stimulants season / dist=poisson offset=l_fu_year link=log;
ods output ParameterEstimates=result;
quit;

data d.irr_&unit._nons; 
set result;
IRR=exp(Estimate);
...
proc sql;
ods output SQL_results=d.incidence_&unit._nons;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "nons&unit" as table
from sccs_&unit.
group by &unit.);
quit;

%mend;
title "nonstimulants analysis";
%nonstimulants(exposure, 0);
%nonstimulants(riskperiod, 0);

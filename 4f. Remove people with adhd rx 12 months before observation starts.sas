...
/*  Main analysis:																*/
/*  inclusion criteria:															*/ 
/*  1. at least one ADHD rx	between 01JAN2001 and 31DEC2020	v					*/
/*	2. aged six and above between 01JAN2001 and 31DEC2020	v					*/
/*	3. at least one suicide attempt dx between 01JAN2001 and 31DEC2020			*/
/*																				*/
/* 	exclusion criteria:															*/
/* 	1. patients with cancer dx 								v					*/
...
order by 1;
quit;
*8)remove patients with adhdrx 12 monts before obsst;
proc sql;
create table b.adhdrx as
select *
from b.rx_data
where exp=1
order by &patientid;
quit;
proc sql;
create table b.adhdrx_2 as
select a.*, b.exp, b.&trtst, b.&trted 
from b.cohort_8 as a left join b.adhdrx as b
on compress(a.&patientid)=compress(b.&patientid);
quit;
data b.adhdrx_3;
set b.adhdrx_2;
if intck('day',&obsst,&trted, 'continuous')<=365 then adhd12mo=1;
else adhd12mo=0;
run;
proc sql;
create table b.adhdrx_4 as
select distinct(&patientid), 
sum(adhd12mo) as adhd12mocount
from b.adhdrx_3
GROUP BY &patientid
order by &patientid;
quit;
data b.adhdrx_5;
set b.adhdrx_4;
where adhd12mocount >0;
run;
proc sql;
create table b.cohort_9 as 
select * from b.cohort_8
where &patientid not in (select &patientid from b.adhdrx_5);
quit;
proc sort data=b.cohort_9 out=b.cohort_final nodupkey; by _all_; run;
...
order by a.&patientid, a.date;
quit;
/*Question 1: Is it ok to have negative age in the model?*/
data b.age_final;
set age1;
if age<6 then agg=1;
...
proc sql;
ods output SQL_results=d.incidence_&unit._sen7;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "sen7&unit" as table
from sccs_&unit.
group by &unit.);
quit;

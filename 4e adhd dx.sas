...
/*	----------------------------												*/
/*  Sensitivity analysis 6:	patients with adhd dx+ADHDrx+suicide				*/
/*  inclusion criteria:															*/ 
...
/* 	4. patients with adhd dx between 01JAN2001 and 31DEC2020					*/
/*																				*/
/* 	exclusion criteria:															*/
...
order by 1;
quit;
*8) select patients with ADHD dx;
proc sql;
create table b.adhddx as
select distinct &patientid, adhd
from b.dx_data
where &adhd;
quit;
proc sql;
...
select *
from b.cohort_8 
where &patientid in (select &patientid from b.adhddx);
quit;
...
*Sensitivity analysis 6: adhd rx+ suicide dx + adhd dx;
%macro sen6(unit, reference);
...
proc sql;
ods output SQL_results=d.incidence_&unit._sen6;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "sen6&unit" as table
from sccs_&unit.
group by &unit.);
quit;

%mend;
title "Sensitivity analysis 6";
%sen6(exposure, 0);
%sen6(riskperiod, 0);

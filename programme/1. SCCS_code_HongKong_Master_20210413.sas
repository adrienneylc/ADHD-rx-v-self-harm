...
from b.cohort1; 
quit; * Patients with ADHD rx from 01JAN2001 to 31DEC2020 = 44271;
...
select count(distinct &patientid) 
from b.cohort2; 
quit; * Patients aged >6 with ADHD rx from 01JAN2001 to 31DEC2020 = 44206;
...
select count(distinct &patientid) 
from b.outcome_all; 
quit; *prevalent patients of outcome = 499;
...
select count(distinct &patientid) 
from b.cohort_final; 
quit; /* final cohort of the study population, n = 406*/
ods rtf close;
/*------------------------------------------------------------------------------*/
...
proc sql;
select atccde,count(distinct &patientid) as tot_pat
from exp_data_desc
group by atccde;
quit;
...
class atccde;
run;
proc means data=b.exp_data_desc median min max q1 q3;
var supply_day;
run;
...
on compress(a.&patientid)=b.&patientid
where compress(a.&patientid) in (select &patientid from b.cohort_final);
quit;
...
left join b.table1 as b 
on compress(a.&patientid)=b.&patientid
where compress(a.&patientid) in (select &patientid from b.cohort_final) and b.&sex="F";
quit;
...
left join b.table1 as b 
on compress(a.&patientid)=b.&patientid
where compress(a.&patientid) in (select &patientid from b.cohort_final) and b.&sex="M";
quit;

...
/*		SCCS_SAS_programme - Analysis 3: Recurrent suicide only					*/
/*		Please refer to separate programme files for analysis 1,2&4				*/
/*																				*/
/********************************************************************************/
option compress=yes;
...
/*	----------------------------												*/
/*  Sensitivity analysis 1:	recurrent suicide									*/
/*  inclusion criteria:															*/ 
/*  1. at least one ADHD rx	between 01JAN2001 and 31DEC2020						*/
/*	2. aged six and above between 01JAN2001 and 31DEC2020						*/
/*	3. at least one recurrent suicide attempt dx between 01JAN2001 and 31DEC2020*/
/*																				*/
/* 	exclusion criteria:															*/
...
/*																				*/
/*  Analysis for recurrent suicide attempt										*/
/* 	A subgroup of patients with at least two suicide attempts 					*/
/*	where the incident and second attempt were recorded during 					*/
/*	the individual’s observational period will be included. 					*/
/*	The follow-up period will begin at 30-day after the incident 				*/
/*	suicide attempt, and the IRRs of subsequent attempt will be 				*/
/*	evaluated with respect to the risk window as defined above, 				*/
/*	using the same definition and analysis approach.							*/
/*------------------------------------------------------------------------------*/
...
/*Note 1: suicide date should be before individual observation end instead of study end*/
proc sql;
...
quit;
*subgroup people with more than one suicide attempt;
proc sql;
create table b.outcome_count as
...
*7) selecting patients with recurrent suicide during the study period from cohort5 -> cohort6;
proc sort data=b.outcome_all_4; by _all_; run; 

data b.outcome_recurrent; 
set b.outcome_all_4; 
by &patientid &admdt; 
if first.&patientid then delete; 
run;
proc sort data=b.outcome_recurrent out=check nodupkey; by &patientid; run; 
*8) Extract first suicide attempt date +30 days as observation start date;
data b.fstoutcome_date; 
set b.outcome_all_4; 
by &patientid &admdt; 
if first.&patientid; 
keep &patientid &admdt; 
run;
data b.obsed;
set b.cohort2_4;
by &patientid; 
if first.&patientid; 
keep &patientid obsed_3; 
run;
proc sql;
create table b.outcome_recurrent_2 as
select a.*, c.&admdt+30 as obsst format date9., b.obsed_3 as obsed
from b.outcome_recurrent as a left join b.obsed as b on a.&patientid=b.&patientid
left join b.fstoutcome_date as c on a.&patientid=c.&patientid 
order by a.&patientid, a.&admdt;
quit;
data b.outcome_recurrent_3; 
set b.outcome_recurrent_2; 
if &obsst<=&admdt<=&obsed;
run; 
data b.outcome_inci;
set b.outcome_recurrent_3;
by &patientid &admdt;
if first.&patientid; 
run;
proc sort data=b.outcome_inci nodupkey; by _all_; run; 
*incident patients of outcome in study period = 94;
...
select distinct b.*
from b.cohort5 as a left join b.outcome_inci as b on a.&patientid=b.&patientid
...
from b.cohort6 as a left join b.outcome_recurrent_3 as b on a.&patientid=b.&patientid
						   left join b.cohort3 as c on a.&patientid=c.&patientid and b.&admdt=c.first_stt
having c.first_stt^=.
order by a.&patientid, c.first_stt;
quit; *outcome and exposure date is equal = 2;
*incident patients of outcome =92 ;
proc sql;
...
order by &patientid;
quit;
* above excluded patients = 2;

proc sql;
create table b.cohort_8 as
select compress(b.&patientid) as id, b.&sex, b.&birth_date, d.&obsst, c.&obsed 
...
left join b.outcome_recurrent_3 as d on compress(a.&patientid)=compress(d.&patientid)
order by 1;
quit;
...
from b.cohort_final; 
quit; /* final cohort of the study population, n = */
...
from b.exp_data as a left join b.cohort2_4 as b on compress(a.&patientid)=compress(b.&patientid)
...
run;
/* Question 2: stt_flag definition ok?*/
data exp_data1; 
set exp_data1; 
by &patientid; 
...
/* dataset for concomitant medication */
...
from b.cohort_final as a left join b.&label._data as b on compress(a.&patientid)=compress(b.&patientid)
...
/*Question3: Note that I use datea instead of obsst*/
proc sql;
...
left join b.outcome_recurrent_3 as b on compress(a.&patientid)=compress(b.&patientid) 
order by a.&patientid, a.date;
quit;
...
proc sql;
ods output SQL_results=d.incidence_&unit._sen1;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "sen1&unit" as table
from sccs_&unit.
group by &unit.);
quit;

%mend;
title "Sensitivity analysis 1: Recurrent suicide";
%sen1(exposure, 0);
%sen1(riskperiod, 0);

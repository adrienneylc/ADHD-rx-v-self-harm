...
/*	----------------------------												*/
/*  Sensitivity analysis 5:	extend exposed period by 10 weeks					*/
/*  inclusion criteria:															*/ 
...
from b.exp_prepres; 
quit; * number of current users aged>6 on study drugs during individual study period =39309 ;
proc sql;
...
select count(distinct &patientid) 
from b.outcome_all; 
quit; *prevalent patients of outcome = 440;
...
*incident patients of outcome in study period = 406;
...
*incident patients of outcome =406 ;
proc sql;
...
proc sql;
ods output SQL_results=d.incidence_&unit._sen5;
select *, event/person_year as incidence 
from (
select &unit., "&unit." as name, sum(outcome) as event, sum(fu_year) as person_year, "sen5&unit" as table
from sccs_&unit.
group by &unit.);
quit;
